...
#define DALOG_FATAL(fmt, arg...)                                        \
    do {                                                                \
        da_asynclog_send(DALOG_LEVEL_FATAL, "[%s:%s:%d]" fmt, __FILE__, \
                     __FUNCTION__, __LINE__, ##arg);                    \
    } while(0)

#define DALOG_WARN(fmt, arg...)                                         \
    do {                                                                \
        da_asynclog_send(DALOG_LEVEL_WARN, "[%s:%s:%d]" fmt, __FILE__,  \
                     __FUNCTION__, __LINE__, ##arg);                    \
    } while(0)

#define DALOG_NOTICE(fmt, arg...)                                       \
    do {                                                                \
        da_asynclog_send(DALOG_LEVEL_NOTICE, "[%s:%s:%d]" fmt, __FILE__, \
                     __FUNCTION__, __LINE__, ##arg);                    \
    } while(0)

#define DALOG_TRACE(fmt, arg...)                                        \
    do {                                                                \
        da_asynclog_send(DALOG_LEVEL_TRACE, "[%s:%s:%d]" fmt, __FILE__, \
                     __FUNCTION__, __LINE__, ##arg);                    \
    } while(0)
...
#define DALOG_DEBUG(fmt, arg...)                                        \
    do {                                                                \
        da_asynclog_send(DALOG_LEVEL_DEBUG, "[%s:%s:%d]" fmt, __FILE__, \
                     __FUNCTION__, __LINE__, ##arg);                    \
    } while(0)
#else
...
int da_synclog_send(const int event, const char *format, ...);

#endif /* _DA_ASYNCLOG_H_ */
